...
- **Live market data** — MGC (Micro Gold) and MNQ (Micro Nasdaq) futures plus SOL (Solana) panels with price, daily change, and intraday sparklines. Futures come from Yahoo Finance via CORS proxies; SOL comes straight from Binance/CoinGecko. Panels show a ● LIVE timestamp or fall back to marked simulation. Ask JARVIS "how's gold?", "solana?", or "market report". Note CME futures are closed Fri 5pm–Sun 6pm ET, so MGC/MNQ sit static over the weekend
